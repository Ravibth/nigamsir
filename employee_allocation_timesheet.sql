...
CREATE MATERIALIZED VIEW IF NOT EXISTS employee_allocation_timesheet TABLESPACE pg_default AS WITH allocation AS (
    ...
    FROM dblink(
            'host=localhost port=5432 user=postgres password=root dbname=Allocation',
            '
...
SELECT ets.employee_mid,
   ...
   CASE
	   ...
	   WHEN ets.proposed_lwd IS NOT NULL 
			AND ets.proposed_lwd < (ets.working_date + COALESCE((SELECT value  FROM dblink('host=localhost port=5432 user=postgres password=root dbname=Configuration'::text, 
	   ...
	   WHEN ets.proposed_lwd IS NULL AND ets.resignation_date IS NOT NULL 
			AND ets.resignation_date < (ets.working_date + COALESCE((SELECT value  FROM dblink('host=localhost port=5432 user=postgres password=root dbname=Configuration'::text, 
				'SELECT "Value" FROM "ApplicationLevelSettings" WHERE "Key" = ''ResignedUserAvailabilityThreshold'' AND "IsActive" = true'::text
			) AS applevelsettings(value text))::int, 0) * INTERVAL '1 day') THEN 0
	   ELSE 8
   END AS capacity,
