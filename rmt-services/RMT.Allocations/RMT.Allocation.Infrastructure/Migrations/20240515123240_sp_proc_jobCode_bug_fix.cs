...
{
    /// <inheritdoc />
    public partial class sp_proc_jobCode_bug_fix : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            string sp = @" -- PROCEDURE: public.sp_jobcode_update(text, text, text, text, text, text)

-- DROP PROCEDURE IF EXISTS public.sp_jobcode_update(text, text, text, text, text, text);
...
BEGIN
	...
	loop
	  ...
	  Insert into public.""Requisition""(""PipelineCode"", ""JobCode"", ""JobName"", ""PipelineName"", ""RequisitionDescription"", ""IsContinuousAllocation"", ""TotalHours"", 
	   ...
	   RETURNING ""RequisionId"" INTO new_requisition_id;
        
	    RAISE NOTICE 'allocations_items.RequisitionId %', new_requisition_id ; 
	   ...
	   /************************** add new resourceAllocation Details *****************************/
	   	total_allocation:= 	(SELECT sum(""ConfirmedPerDayHours"")
			FROM public.""ResourceAllocationDays"" WHERE ""RequisitionId"" = resource_allocation_details_rows.""RequisitionId"" 
							 and  ""ConfirmedAllocationStartDate"" > yesterday);
		RAISE NOTICE 'ResourceAllocation->total_allocation-01 %', total_allocation;
		RAISE NOTICE 'new_requisition_id-01 %', new_requisition_id;
		 IF total_allocation IS NOT NULL THEN
                ...
                SELECT  new_pipeline_code, new_job_code, new_job_name, ""PipelineName"", ""EmpEmail"", ""EmpName"", new_requisition_id, ""RecordType"", 
                ...
                FROM public.""ResourceAllocationDetails"" 
                WHERE ""Id"" = resource_allocation_details_rows.""ResAllocDetailsId""
	   ...
	   Insert into public.""ResourceAllocation""(""PipelineCode"", ""JobCode"", ""JobName"", ""EmpEmail"", ""EmpName"", ""ConfirmedAllocationStartDate"", 
											   ...
											   ""isPerDayHourAllocation"", ""SuspendedAt"", ""isPublish"", ""RatePerHour"")
	   	select new_pipeline_code, new_job_code, new_job_name, ""EmpEmail"", ""EmpName"", cDate,  ""ConfirmedAllocationEndDate"", total_allocation, 
												new_requisition_id, ""AllocationStatus"", ""RecordType"", ""CreatedDate"", ""ModifiedDate"", ""CreatedBy"", 
		...
		RAISE NOTICE 'total_allocation-02:%',yesterday;
	 ...
	 UPDATE public.""ResourceAllocationDetails""  
			...
			WHERE 
			public.""ResourceAllocationDetails"".""RequisitionId"" = public.""ResourceAllocation"".""RequisitionId"" and
    ...
    Update ""ResourceAllocation"" set ""PipelineCode""=new_pipeline_code,  ""JobCode"" = new_job_code, ""JobName"" = new_job_name , ""ModifiedBy"" = modified_by 
	where ""IsActive"" = true 
	and ""ConfirmedAllocationStartDate"" >= cdate 
        and ""PipelineCode"" = pipeline_code and ""JobCode"" = job_code; 

	Update  public.""ResourceAllocationDays"" set ""PipelineCode""=new_pipeline_code, ""JobCode"" = new_job_code, ""JobName"" = new_job_name,
	""ModifiedBy"" = modified_by  
	 				where ""PipelineCode"" = pipeline_code and ""JobCode"" = job_code
