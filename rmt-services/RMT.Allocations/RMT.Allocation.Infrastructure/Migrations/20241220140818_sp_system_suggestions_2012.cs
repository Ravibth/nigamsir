...
{
    /// <inheritdoc />
    public partial class sp_system_suggestions_2012 : Migration
    {
        /// <inheritdoc />
...
-- 						Checking for offerings
						...
						then
							...
							then
								...
								values (
									requisition_parameters->>'Category'
									,CEILING(parameter_weightage * pref_weightage_constraint)
									,preferred_match
									,requisition_parameter_value
								);
...
-- 	        		Constructing the JSONB object
				...
				score_calculated := CEILING(((select sum(value) from score_breakups_table) :: decimal/max_score :: decimal)*100) ;
			end if;
