...
namespace ServiceLayer.HelperMethod
{
    public class ProjectBudgetHelper
    {
            ...
            if (allProjects != null)
            {
                ...
                foreach (ProjectDTO project in allProjects)
                {
                    try
                    {
                        ...
                        string buOfferingKey = $"{buName}{Constants.Constants.SeparatorPipe}{offerings}";
                        ...
                        if (jobCode != null && project.EndDate != null && project.StartDate != null)
                        {
                            ...
                            {
                                warningConfig = await _configurationService.GetConfigurationByConfigGroupType(buOfferingKey, Constants.Constants.ConfigGroupExpertise.WARNING_ALERT_CONDITION, currentToken, _logger);
                                ...
                                _logger.LogInformation("--BudgetSchedular--GetConfigurationByConfigGroupType--Request--{0}--{1}--Response--{2}",
                                    buOfferingKey, Constants.Constants.ConfigGroupExpertise.WARNING_ALERT_CONDITION, JsonConvert.SerializeObject(warningConfig));
                            ...
                            {
                                alertAllocationConfig = await _configurationService.GetConfigurationByConfigGroupType(buOfferingKey, Constants.Constants.ConfigGroupExpertise.BUDGET_ALERT_CONDITION_FOR_ALLOCATION, currentToken, _logger);
                                _logger.LogInformation("--BudgetSchedular--GetConfigurationByConfigGroupType--Request--{0}--{1}--Response--{2}",
                                    buOfferingKey, Constants.Constants.ConfigGroupExpertise.BUDGET_ALERT_CONDITION_FOR_ALLOCATION, JsonConvert.SerializeObject(alertAllocationConfig));
                                ...
                                if (warningConfig != null)
                                {
                                    ...
                                    if (alertAllocationConfig != null)
                                    {
                                        ...
                                        if (isOutLimit && alertNotificationLimit > 0)
                                        {
                                            ...
                                            {
                                                action = Constants.Constants.PROJECT_BUDGET_NOTIFICATION,
        ...
        public async Task<List<ProjectDTO>> GetAllProjectList(string currentToken, ILogger _logger)
        {
            using (var client = AzureHttpClient.GetAzureHttpClient(true))
            {
                ...
                var getAllPath = Convert.ToString(environmentVaribles[Constants.Constants.EnvAppSettingConstants.GetAllProjectsForBudget]);
                var gateway = Convert.ToString(environmentVaribles[Constants.Constants.EnvAppSettingConstants.GET_GATEWAY_BASE_URL]);
        ...
        public async Task<List<ProjectDTO>> GetAllProjectsForBudgetByJobCodes(List<string> jobCode, string currentToken, ILogger _logger)
        {
            using (var client = AzureHttpClient.GetAzureHttpClient(true))
            {
                ...
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", currentToken);
                client.DefaultRequestHeaders.Add("Accept", "Application/JSON");
                var getAllPath = Convert.ToString(environmentVaribles[Constants.Constants.EnvAppSettingConstants.GetAllProjectsForBudgetByJobCodes]);
                var gateway = Convert.ToString(environmentVaribles[Constants.Constants.EnvAppSettingConstants.GET_GATEWAY_BASE_URL]);
                var path = gateway + getAllPath;
        ...
        public static async Task<List<BudgetOverViewDto>> GetProjectBudgetStatus(string currentToken, BudgetOverviewRequest request, ILogger _logger)
        {
            using (var client = AzureHttpClient.GetAzureHttpClient(true))
            {
                ...
                var BudgetOverviewPath = Convert.ToString(environmentVaribles[Constants.Constants.EnvAppSettingConstants.BUDGET_OVERVIEW]);
                var gateway = Convert.ToString(environmentVaribles[Constants.Constants.EnvAppSettingConstants.GET_GATEWAY_BASE_URL]);
        ...
        public static async Task<List<ProjectDTO>> UpdateProjectBudget(string currentToken, List<UpdateBudgetStatusDTO> updateBudgetRequest, ILogger logger)
        {
            try
            {
                using (var client = AzureHttpClient.GetAzureHttpClient(true))
                {
                    ...
                    var requestData = new StringContent(json, Encoding.UTF8, "application/json");
                    var environmentVaribles = Environment.GetEnvironmentVariables();
                    var UpdateProjectBudgetStatusPath = Convert.ToString(environmentVaribles[Constants.Constants.EnvAppSettingConstants.UPDATEPROJECT_BUDGET]);
                    var gateway = Convert.ToString(environmentVaribles[Constants.Constants.EnvAppSettingConstants.GET_GATEWAY_BASE_URL]);
        ...
        public async Task<String> GetProjectActualBudgetOverShoot(string currentToken, ILogger logger)
        {
            try
            {
                using (var client = AzureHttpClient.GetAzureHttpClient(true))
                {
                    ...
                    var GetGatewayBaseUrl = Convert.ToString(EnvVarDictionary[Constants.Constants.EnvAppSettingConstants.GET_GATEWAY_BASE_URL]);
                    var ProjectActualBudgetOverShoot = Convert.ToString(EnvVarDictionary[Constants.Constants.EnvAppSettingConstants.PROJECT_ACTUAL_BUDGET_OVERSHOOT]);
        ...
        {
            string type = Constants.Constants.NotificationTypeNotification;
