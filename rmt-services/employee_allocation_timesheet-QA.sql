...
AS
 WITH allocation AS (
         SELECT allocation.email_id,
           ...
           FROM dblink('host=10.152.1.36 user=rmsdevdba password=K$u7!6fa(p_yMt dbname=devrms_allocation'::text, '
         ...
         SELECT holidays.holiday_date,
           ...
           FROM dblink('host=10.152.1.36 user=rmsdevdba password=K$u7!6fa(p_yMt dbname=devrms_wcgt'::text, '
